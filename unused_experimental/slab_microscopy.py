...
'''
Microscopy toolbox of the Schönwiesner Lab.
'''
...
def artificial_data(edge_length=500,n_cells=50,cell_radius_range=[5,7,10,15],cell_intensity=100,background_intensity=50,noise_meter=0):
    '''
...
def _gabor_shell(edge_length=50, radius=6, sigma=None, freq=.1, phase=4.7124, z_scale_factor=1):
    ...
    kernel = (1/A) * numpy.exp(-1 * numpy.pi * (r-radius)**2 / sigma**2) * numpy.cos(2 * numpy.pi * freq * (r-radius) + phase)
...
def gabor_shell(edge_length=50, radius=6, sigma=None, freq=.1, z_scale_factor=1, plot=False):
    ...
    kernel = _gabor_shell(edge_length=edge_length, radius=radius, sigma=sigma, freq=freq, phase=phase, z_scale_factor=z_scale_factor)
    if plot:
        fig, ax = plt.subplots()
...
def _compute_phase(edge_length=50, radius=6, sigma=None, freq=.1, z_scale_factor=1, plot=False):
    ...
    Internal function. Don't call directly.
    '''
    gabor = functools.partial(_gabor_shell, edge_length=edge_length, radius=radius, sigma=sigma, freq=freq, z_scale_factor=z_scale_factor)
...
def interactive_gabor_shell(edge_length=50, radius_0=6, sigma_0=8,freq_0=0.1,phase_0=4.7124):
    ...
    def sliders_on_changed(val):
        data = _gabor_shell(edge_length=edge_length,radius=radius_slider.val, sigma=sigma_slider.val, freq=freq_slider.val, phase=phase_slider.val)[edge_length//2,:,:]
        data = data/data.max()
        img.set_data(data)
...
def rotate_dataset(cellcoords,n_cell,surfacecoords,):#unfinished, do not use
    ...
    gamm=m.acos((np.dot(v_ab_xy,v_y))/(m.sqrt(v_ab_xy[0]*v_ab_xy[0]+v_ab_xy[1]*v_ab_xy[1])*m.sqrt(v_y[0]*v_y[0]+v_y[1]*v_y[1])))
    ...
    alph=m.acos((np.dot(v_ab_yz,v_z))/(m.sqrt(v_ab_yz[0]*v_ab_yz[0]+v_ab_yz[1]*v_ab_yz[1])*m.sqrt(v_z[0]*v_z[0]+v_z[1]*v_z[1])))
    ...
    if plot == True:#plot 3D scatterplot
        ...
        for i in range(len(labels)):
            ...
            ax.text(coord_rotated[i,0],coord_rotated[i,1],coord_rotated[i,2],  '%s' % (labels_r[i]), size=10, zorder=1,)
        ax.set_zlabel("Z")
        ax.set_ylabel("Y")
        plt.show()
...
def preprocessing_terastitcher(input_path, output_path, raw_filename,n_tiles_x=8, n_tiles_y=3,n_tiles_z=801, channels=(r"C00", r"C01"), input_format=r".tif"):
    """
    ...
    # while n_iter < 1:
        ...
        for channel in range(len(channels)): #iterate through channels
            ...
            for z in range(final_shape[0]): #iterate through z
                ...
                current_z = tf.imread(os.path.join(input_path, prefix_channel) + z_number + suffix, multifile = False) #adding Z index to filename and opening the relevant file
