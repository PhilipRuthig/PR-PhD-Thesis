...
import math
'''
semirigid registration pipeline. Don't use sITK.
'''
...
overview_img_path = r"F:\Philip\AJ001\Registration\AJ001_overview_downscaled_further.tif" #Overview microscopy image.
hires_img_path = r"F:\Philip\AJ001\Registration3\C00_detail_downscaled.tif" #step3: fixed img, not downscaled yet
...
hires_img_path = r"C:\Data\AJ003\AJ001_highres_downscaled_edge.tif" #step3: fixed img, downscaled
...
if step3 == True: ### affine alignment of registered overview image to downscaled high resolution image
    ...
    rotated_mov = np.rot90(sitk.GetArrayFromImage(fixed_image),k=3,axes=(0,1))
    ...
    rotated_mov = sitk.GetImageFromArray(rotated_mov.astype("uint16"))
    rotated_atl = sitk.GetImageFromArray(rotated_atl)

    #load images
    moving_image = rotated_mov #fixed img from last step is now moving img
    fixed_image = downscaled_hiresimg.astype("uint16")
    atlas_raw = rotated_atl

    #forget rotated and hires_img to save RAM
    rotated_mov,rotated_atl,hires_img = None,None,None
    ...
    # Similarity metric settings.
    registration_method.SetMetricAsMattesMutualInformation(numberOfHistogramBins=150) #default:100 // 2500
    registration_method.SetMetricSamplingStrategy(registration_method.REGULAR)#.RANDOM
    registration_method.SetMetricSamplingPercentage(0.3) #default: 0.01 // 0.15 #0.3
    ...
    registration_method.SetOptimizerAsGradientDescent(learningRate=0.04, numberOfIterations=400, convergenceMinimumValue=1e-6, convergenceWindowSize=40)
    registration_method.SetOptimizerScalesFromPhysicalShift()
    #for AJ001:learningrate 0.04

    # Setup for the multi-resolution framework.            
    registration_method.SetShrinkFactorsPerLevel(shrinkFactors =   [4,2,1])#8,4,2])#16,8,4,2,1
    registration_method.SetSmoothingSigmasPerLevel(smoothingSigmas=[3,1,0])#4,3,2])#54310
    registration_method.SmoothingSigmasAreSpecifiedInPhysicalUnitsOn()
